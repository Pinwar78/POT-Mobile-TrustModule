...
public class NodeRequest {
    ...
    public NodeRequest(Context context, String host, String nodePubKey) {
        this.context = context;
        this.nodeSettings = new NodeSettingsService(context);
        this.host = host;
    ...
    private JSONObject encryptData(String dataToEncrypt) throws NoSuchAlgorithmException {
        ...
        try {
            ...
            Log.d("DecryptedWithAES", dataToEncrypt);
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(requestMethod, url, encryptedDataJson, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                ...
                try {
                    ...
                    AESDecryptor aesDecryptor = new AESDecryptor(rsaDecryptor.decryptData(response.getString("aeskey")), response.getString("iv"));
    ...
    public void getBytes(final String endpoint, final int requestMethod, String dataToEncrypt, final INodeRequestCallback<byte[]> callback) throws Exception{
        if (host.equals("")){
            Toast.makeText(context, "Please connect to a PoT node.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        BytesVolleyRequest fileRequest = new BytesVolleyRequest(requestMethod, url, encryptedDataJson,
                    ...
                    public void onResponse(byte[] response) {
                        try {
                            if (response != null) {
                                ...
                                Toast.makeText(context, "Dapp download complete.", Toast.LENGTH_LONG).show();
                            }
                        } catch (Exception e) {
                            Log.d("POTT ERROR", "UNABLE TO DOWNLOAD FILE");
                            e.printStackTrace();
                        }
                    }
                },
        ...
        fileRequest.setTag("myfilerequest");
        queue.add(fileRequest);
    }

    public void getBytesDapp(final String endpoint, final int requestMethod, String dataToEncrypt, final INodeRequestCallback<byte[]> callback) throws Exception{
        if (host.equals("")){
            Toast.makeText(context, "Please connect to a PoT node.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        String url = "http://"+host+":9070"+endpoint;
